...
| page 部品 | `create_ui_page` (テーマ + CSS 注入 + portal drain の要 + `.ric-page` への bg/fg/font-size 塗り) | **廃止** (役割は `applyTheme` / CSS 1 枚 / portal ホストに分解)。`create_ui_page` が塗っていた bg/fg/font-size は `applyTheme` した要素自身に塗られる形でパリティ確保 (bg/fg は alpha.3〜、font-size は alpha.6〜。それまでは該当プロパティが要素自身に塗られない欠落があった) |
