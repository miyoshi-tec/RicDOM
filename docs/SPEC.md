...
- **descriptor**: `{ v?, s?, p }` — `v`=viewBox(既定 `0 0 24 24`)、`s`=stroke-width。
  **stroke がデフォルト**(省略=2 / 数値=その太さ / `null`=fill モード)。`p`=path の
  `d` 文字列 or 配列。
- **色**: `currentColor` 固定。CSS の `color` でテーマに追従する。
- **size**: 数値(px)or CSS 文字列。既定 `'1em'`(= 親 font-size 追従)。
- **label**: 指定で `role="img"` + `aria-label`、省略で `aria-hidden="true"`
  (隣にテキストがある装飾アイコンの二重読み上げを防ぐ)。
- **spin**: `true` で `@keyframes ric-spin` による回転(spinner 用)。
- **strokeWidth**: `descriptor.s` の上書き(16px で線が太い時など)。
- ボタン内に置くと `ric-button` の `gap` で自動的にテキストと横並び・中央揃え。
- **生 RicDOM(`create_ui_page` 不使用)で使う場合**: `size` と
  `vertical-align`(テキスト隣接時のベースライン整列)は **inline style** なので
  クラス無しでも効く。一方 `flex-shrink: 0` や `spin` の `@keyframes` は
  `.ric-icon` CSS(= `create_ui_page` が注入)側にあるため効かない。
  flex コンテナ内で潰れない/回転させたい場合は、`create_ui_page` を使うか、
  consumer 側で `.ric-icon { flex-shrink: 0 }` 等を用意する。
  テキストと縦中央で揃えたいだけなら、親に `display: inline-flex; align-items: center`
  を当てるのが手軽(v0.3.29〜は `vertical-align` が inline 化され、素の inline 配置でも
  概ね揃う)。
