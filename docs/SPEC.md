...
```javascript
const ICONS = {
  check: { s: 2, p: 'M20 6 9 17l-5-5' },   // s=stroke幅 → 線画 (Lucide 系)
  heart: { p: 'M12 21 ...' },               // s 無し → 塗り (solid 系)
  menu:  { s: 2, p: ['M4 6h16', 'M4 12h16', 'M4 18h16'] },  // 複数 path
};
...
- **descriptor**: `{ v?, s?, p }` — `v`=viewBox(既定 `0 0 24 24`)、`s`=stroke-width
  (あれば stroke モード、無ければ fill モード)、`p`=path の `d` 文字列 or 配列。
