...
//
// 安全性:
//   - 実際には全 template literal に作用する (バッククォート対の中身全体が対象で、
//     「CSS テキスト部分のみ」ではない)。RicUI bundle の literal はほぼ CSS なので
//     許容している。`${...}` (JS 式部分) はそのまま残るので破壊されない。
//     `` \` `` を含む literal では対応がズレる可能性があるが、現 bundle には
//     存在せず無害 (v03_lz_bundle テストで export 同一性を固定済み)。
