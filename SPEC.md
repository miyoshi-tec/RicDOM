...
- **descriptor は `<path>` 専用**。Lucide 等の生 SVG に `<line>` / `<circle>` /
  `<rect>` / `<polyline>` 等が含まれる場合はそのまま `p` に入れられない。
  アイコンピッカー経由なら自動で path に変換されるが、生 SVG を手で拾うときは
  `docs/icons/svg_to_descriptor.js` を通して descriptor 化する。
